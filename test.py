...
def loginpage(logdata):
    ...
    entry1 = customtkinter.CTkEntry(master=frame,textvariable=uname, placeholder_text="Username")
    ...
    def check():
        for a, b, c, d in logdata:
            if b == uname.get() and c == passw.get():
                print(logdata)
    ...
    # LOGIN BUTTON
    log = Button(frame, text='Login', padx=5, pady=5, width=5, command=check, fg="white", bg="black")
...
def signUpPage():
    ...
    entry1 = customtkinter.CTkEntry(master=frame2,textvariable=uname, placeholder_text="Username")
    entry1.pack(pady=12,padx=10)
    ...
    option = customtkinter.CTkOptionMenu(frame2, values=["India", "USA", "Japan", "Pakistan", "SriLanka", "India", "USA",
                                         ...
                                         dropdown_text_color="black", fg_color="black",dropdown_fg_color="white" ,variable=ctry)
    ...
    def addUserToDataBase():
        ...
        if len(fname.get()) == 0 and len(uname.get()) == 0 and len(passw.get()) == 0 and len(ctry.get()) == 0:
            error = Label(text="You haven't enter any field...Please Enter all the fields", fg='black', bg='white')
            error.place(relx=0.37, rely=0.7)

        elif len(fname.get()) == 0 or len(uname.get()) == 0 or len(passw.get()) == 0 or len(ctry.get()) == 0:
            error = Label(text="Please Enter all the fields", fg='black', bg='white')
            error.place(relx=0.37, rely=0.7)

        elif len(uname.get()) == 0 and len(passw.get()) == 0:
    ...
    # signup BUTTON
    sp = Button(frame2, text='SignUp', padx=5, pady=5, width=5, command=addUserToDataBase, bg="black", fg="white")
